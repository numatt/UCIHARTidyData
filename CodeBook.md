# General Notes
* From the original data set, we only retain features that are immediate means or standard deviations (those whose name end with "-mean()" or "-std()" in the original data set). We do not include fields with further calculations of means or stds
